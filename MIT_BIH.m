...
arrhy_list = [100 103 231 113 115 221 117 219 220 234];
arrhy_path = 'matlab_data/MIT_BIH_Arrhythmia';
normal_list = [16265 16272 16273 16483 16539 16773 16786 16795 17052 18184 19088 19090 19093 19140];
normal_path = 'matlab_data/MIT_BIH_Normal_Sinus_Rhythm';
data_path = normal_path;
data_list = normal_list;
for class = 1:length(data_list)
% for class = 8:8
%     fs = 360;
%     M = rddata(data_list(class),data_path);   %将MIT-BIH上的数据转换成矩阵格式

    % raws=M(1:65536,1);    % 取第一组数据的前65536个数,分解后的cd3长度为8194
%     raws = M(1:65536*8,1);    % 取第一组所有数据

    [M,fs] = MIT_BIH_rdata(data_list(class),data_path);
    sfreq = fs;
    raws = M(1:65536*8,1);
